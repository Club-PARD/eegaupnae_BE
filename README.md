...
단순한 가격 비교를 넘어, Gemini AI를 활용하여 상품의 **실질적 가치**(성분, 용량, 브랜드 신뢰도 등)를 종합적으로 분석합니다. 소비자가 오프라인 마트에서 현명한 구매 의사결정을 내릴 수 있도록 **픽리포트**를 통해 즉각적인 인사이트를 제공합니다.
...
- **VFM(Value for Money) 분석**: 상품의 실질 가치와 가격을 비교하여 **0.0~5.0점**의 픽스코어를 산출
...
| Method | Endpoint | 설명 |
|--------|----------|------|
| `POST` | `/scan` | 상품 스캔 및 저장 |
| `GET` | `/scan?userId={id}` | 스캔 목록 조회 |
...
| [erd_diagram.png](erd_diagram.png) | 데이터베이스 ERD 다이어그램 |
